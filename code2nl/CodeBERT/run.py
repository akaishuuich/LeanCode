...
def load_leancode_weights():
    ...
    return  all_weight_dicts['code2nl_codebert']
...
def main():
    ...
    # Other parameters
    parser.add_argument("--prune_strategy", default="None", type=str,choices=['leancode', 'dietcode', 'leancode_d','None'],
    ...
    if args.gen_weight:
        dic={}
        wo = WeightOutputer()
        wo.set_output_file_dir('./code2nl/CodeBERT/weights')
        eval_examples = read_examples(args.train_filename)
        eval_features = convert_examples_to_features(eval_examples, tokenizer, args, dic, stage='dev')
        all_source_ids = torch.tensor([f.source_ids for f in eval_features], dtype=torch.long)
        all_source_mask = torch.tensor([f.source_mask for f in eval_features], dtype=torch.long)
        all_target_ids = torch.tensor([f.target_ids for f in eval_features], dtype=torch.long)
        all_target_mask = torch.tensor([f.target_mask for f in eval_features], dtype=torch.long)
        ...
        for f in eval_features:
            example_ids.append(f.example_id)
        all_index = torch.tensor(example_ids, dtype=torch.long)
        eval_data = TensorDataset(all_source_ids, all_source_mask, all_target_ids, all_target_mask, all_index)
        eval_sampler = SequentialSampler(eval_data)
        eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
