...
def main():
    ...
    if args.do_train:
        ...
        train_examples, train_data = load_and_cache_gen_data(args, args.train_filename, pool, tokenizer, 'train',weight_dicts)
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            if args.do_eval:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples, eval_data = load_and_cache_gen_data(args, args.dev_filename, pool, tokenizer, 'dev',weight_dicts)
    ...
    if args.do_test:
        ...
        for criteria in ['best-bleu']:
            ...
            #     model = torch.nn.DataParallel(model)
            model.load_state_dict(torch.load(file))
            eval_examples, eval_data = load_and_cache_gen_data(args, args.test_filename, pool, tokenizer, 'test',weight_dicts,
    ...
    if args.gen_weight:
        wo = WeightOutputer()
        wo.set_output_file_dir("./code2nl/CodeT5/weights")
        logger.info("  " + "***** Output utils *****")
        logger.info("  Batch size = %d", args.eval_batch_size)
        assert args.load_model_path !='None'
        # path=args.load_model_path
        # logger.info("Reload model from {}".format(path))
        # model = T5ForConditionalGeneration.from_pretrained(args.model_name_or_path)
        ...
        if args.n_gpu > 1:
            ...
            model = torch.nn.DataParallel(model)
        # model.load_state_dict(torch.load(file))
        eval_examples, eval_data = load_and_cache_gen_data(args, args.train_filename, pool, tokenizer, 'dev',
                                                           weight_dicts)
        eval_sampler = SequentialSampler(eval_data)
        eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size,
                                     num_workers=4, pin_memory=True)
